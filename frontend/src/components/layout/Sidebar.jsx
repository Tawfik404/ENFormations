...
import { Home, Users, UserPlus, BookOpen, Layers, Building2 } from 'lucide-react';
 
const NAV = [
  { to:'/dashboard',        label:'Dashboard',          icon: Home },
  { to:'/apprenants',       label:'Liste Apprenants',   icon: Users },
  { to:'/ajouter-apprenant',label:'Ajouter Apprenant', icon: UserPlus },
        ...
        {NAV.map(item => {
          ...
          <NavLink key={item.to} to={item.to}
            style={({ isActive }) => ({
              ...
              background: isActive?`rgba(79,255,176,0.15)`:'transparent',
              color: isActive?accent:'#e8eaf0',
